...
2. メッセージ到達 → **復号 → トースト表示**。アクションボタン付きの本式な通知を v1 から出す。
   ...
   - 上段に出すのは**押した結果が発出元で完結するアクションだけ**とする（`actionDetails`（§4.1）の分類による）。発出元で画面が開くだけのものと、分類する材料が無いもの（旧バージョンから届いたペイロード）は出さない。手元で結果を確かめられない操作をトーストへ並べても空振りになるため。出さなかったアクションもタイムライン（§10.1）には並ぶので、操作する手立ては残る。載せるアクションが無い通知は上段ごと出さない。
