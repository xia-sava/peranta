...
- Android 端末に来た通知（最低限 OTP / ワンタイムコード）を、他の端末で見られる（典型はスマホから Windows PC / Android タブレットへ）。
...
5. 元通知を指せる（`notificationKey` を持つ）通知には「送信元の通知を消す」アクションを付ける。
   押すとアプリを開かずに既読同期（§3.4）を発火し、元端末と他の受信端末から消える。あわせて
   自端末のミラー通知も取り下げる。対応づいていない SMS・受信メッセージは対象を指せないため
   付けない。Desktop のトースト（§3.3）と同じ操作で、プラットフォーム間で体験を揃える。
6. 本文（通知は title + text、SMS は text）から URL が抽出できたときは、通知に ACTION_VIEW の「開く」
   アクションを 1 個付ける。押すと受信端末のローカル操作として既定ブラウザで先頭 URL を開く
   （コマンドは送らない）。
7. 通知本体（「開く」ボタンではなく通知全体）をタップするとアプリを起動し、タイムラインの該当
...
8. 通知には**発信元**（転送元の端末名と、その端末で通知を出したアプリ名）を添える。タイトルの
...
9. 通知へ渡る文字列は受信の関門で正規化済みのものを使う（§4.4）。発信元とタイトル・本文の境界を
   改行や不可視文字で曖昧にされないよう、表示面ごとに手当てせず 1 箇所で当てる。
10. ミラー通知は**元通知のロック画面可視性（§4.1 の `visibility`）に従う**。伏せる指定のときは
...
2. メッセージ到達 → **復号 → トースト表示**。アクションボタン付きの本式な通知を v1 から出す。
   ...
   - ボタンは**上段に元通知のアクション、下段にこの端末の操作**（「開く」「送信元の通知を消す」）を置いた 2 段構成にする。押した先が発出元かこの端末かで段を分けることで、通知の中身によらず「送信元の通知を消す」がいつも同じ位置に来る。
   - 下段は、本文（title + text）から URL が抽出できたときは「開く」ボタンを追加した 2 ボタン構成にする。「開く」押下時は既定ブラウザで先頭 URL を開く（受信端末のローカル操作のみで、コマンドは送らない）。URL が無い通知は「送信元の通知を消す」の 1 ボタン構成にする。
   - 上段に出すのは**押した結果が発出元で完結するアクションと、この端末で本文を入力して送るアクション（インライン返信）だけ**とする（`actionDetails`（§4.1）の分類による）。発出元で画面が開くだけのものと、分類する材料が無いもの（旧バージョンから届いたペイロード）は出さない。手元で結果を確かめられない操作をトーストへ並べても空振りになるため。出さなかったアクションもタイムライン（§10.1）には並ぶので、操作する手立ては残る。載せるアクションが無い通知は上段ごと出さない。
   - 上段は**3 個まで**とする。トーストの幅を等分して 1 行に収めるため、ラベルが読める幅を保てる数で止める。あふれた分はタイムラインで操作する。ボタンのラベルは元通知のアクション名をそのまま出す（実行先はヘッダの発信元表示で判るため、タイムライン（§10.1）のような接頭辞は付けない）。
   - 上段の押下は元通知のアクション位置を指す invokeAction（§3.4）を発出元へ送る。トーストは操作するまで残るため、その間に元通知が差し替わってアクションの並びが変わることがある。**表示したときの名前と、押した時点の同じ位置の名前が食い違うなら送らない**。位置だけで発火すると、利用者が見ていたのとは別の操作を起こしてしまうため。
   - **インライン返信のアクションだけは、コマンドを送らずタイムラインへ引き継ぐ。** トーストは本文の入力を受けないため、押すとウィンドウを前面化し、そのアイテムまでスクロールして返信入力を開く（§10.1）。名前の照合は他のアクションと同じく先に行い、食い違うときは引き継がない。
   ...
   - × とスワイプは**既定ではこの端末の表示を引っ込めるだけ**でコマンドを送らない。転送元に通知を残したままこの端末の画面だけ片づける使い方（手元で処理する用事のリマインダー等、転送元に通知が残っていること自体に意味がある通知）を支えるため、「送信元の通知を消す」との違いは意図した仕様として維持する。この区別はジェスチャの意味で決まり、アイテムの種類には依らない。
   - **払いのけたときに元通知も消すかは、送信端末がアプリごとに決めてペイロードで指示する（`swipeBehavior`、§4.1 / §7）。**
     指示があるアプリの通知は、× とスワイプが「送信元の通知を消す」と同じ既読同期（§3.4）も起こす。
     指示を運ばない（旧バージョン由来の）通知と元通知を持たないアイテムは既定の「表示を引っ込めるだけ」に
     なる。受信端末ごとに設定を持たず送信端末の判断を配ることで、どの受信端末でも同じ挙動になる。
     Android のミラー通知では、指示があるときだけ払いのけを受け取る intent を通知に付ける（§3.2）。
...
3. OS 起動時オートスタート登録。設定画面の「自動起動」から登録・解除する（§10.2）。
...
### 3.4 逆方向チャネル：受信端末から転送元の通知を操作する

受信端末から、転送元の端末の通知に対する操作コマンドを送れる。

1. 転送元も受信端末と同じく UnifiedPush で自分のエンドポイントを持つ（同一 APK の送受信兼用構成そのまま）。
2. 受信端末は操作コマンド（type: `command`）を暗号化して転送元のエンドポイント topic へ POST。
3. 転送元は broadcast で起こされ、NLS の API で対象通知を操作する。
...
アプリごとの転送設定（§7）も受信端末から変えられる。`muteApp` / `unmuteApp` は転送可否だけを切り替え、
`setAppRule` は扱い（転送可否・優先度上書き・伏せ字・払いのけの扱い）を 1 つの値としてまとめて置き換える。
設定はどれも発信側に効くため、これらは元通知への操作ではなく発信側の設定変更として扱う。
...
コマンドには失効時刻（`expiresAtEpochMillis`）を付けて受信側が期限切れを無視できるようにし、あわせて
publish 時の `Cache` ヘッダでサーバの保持時間を決める。この 2 つは、遅れて届いたときに何が起きるかで
種別ごとに分ける。

| 分類 | 対象 | 失効 | サーバ保持 |
|------|------|------|-----------|
| 即時操作 | アクション発火・返信 | 2 分 | 60 秒 |
| 状態同期 | 消す・mute / unmuteApp | 24 時間 | 指定せず `cache-duration`（§9）に委ねる |

- 即時操作は遅れて実行されると誤爆する（半日前の返信が今になって送られる等）ため短く切る。返信本文が
  サーバに残る時間も同じだけに抑える。
- 状態同期は遅れて届いても結果が変わらない。消すは冪等で、対象が既に消えていれば空振りする。
  mute / unmuteApp は設定変更のため、届かなければ恒久的に失われる。受信端末が半日単位で離れていること
  （朝届いた通知を夕方に見る）は通常の運用なので、その間に他端末で起きた消去を再接続時に受け取れる
  必要がある。
- 受信端末は再接続時に `since` で取りこぼしを回収する。回収にはサーバにメッセージが残っていることと
  失効していないことの両方が要るため、保持と失効は揃えて決める。
...
どこか 1 端末で消したら、転送元の元通知と他の受信端末の表示もまとめて消える。

- 受信端末で「送信元の通知を消す」（トーストのボタン / タイムラインの ✓ ボタン）→ dismiss コマンドを全端末宛（`to: "*"`）に送る。転送元は元通知を `cancelNotification`、他の受信端末は自分が表示した通知を取り下げる。
- 転送元で元通知が消えた時も、NLS の `onNotificationRemoved()` で検知して同じ dismiss を全受信端末へ送る。
- 転送元でアクション発火・返信の対象通知が見つからず実行できなかったときも、同じ dismiss を
...
- 転送元は、対象が既定 SMS アプリの通知なら「既読にする」アクション（`semanticAction` が
...
3. 各端末は control トピックのメッセージ履歴（ntfy の cache を `since=all` で HTTP 取得可）からロスターを構築・更新する。
   - 送信側の端末は永続接続を持たないが、送信時や WorkManager の定期ジョブでの HTTP ポーリング取得で足りる。
...
- `swipeBehavior`（optional）: 受信端末でミラー表示を払いのけたときの扱い（`localOnly` /
  `dismissSource`）。送信端末がアプリごとの設定（§7）から決めて載せ、受信端末はこれに従う（§3.3）。
  旧バージョン由来のペイロードでは持たず、受信側は `localOnly` として扱う。値を増やせば
  「払いのけたら履歴からも消す」のような扱いも同じ仕組みで配れる。
...
```json
{
  "command": "dismiss | invokeAction | reply | muteApp | unmuteApp | setAppRule",
...
`appRule` は `setAppRule` だけが運ぶ、アプリごとの扱い（§7）。転送可否・優先度上書き・伏せ字・
払いのけの扱いをまとめて置き換える。項目を増やしてもコマンド種別を増やさずに済ませるため、
個別のフィールドではなく 1 つの値として送る。
...
- 方式: **対称鍵 AEAD（AES-GCM 256bit・ランダム 12 バイト nonce）を全端末で共有**。実装は cryptography-kotlin。
  - 共有鍵はリポジトリ・ビルド成果物・サーバのいずれにも置かない。初回セットアップ時に設定元の端末で生成し、各端末のローカル設定ストレージにのみ保存する（端末間の受け渡しは QR のみ）。
  ...
  - 鍵配布は **QR コードによるペアリング**。設定元の端末が QR を表示し、他の端末のカメラで読み取る
    （設定元は「設定元にする」で選ぶ。トークン発行等の手作業をする都合で Windows PC になることが多い）:
    1. サーバ側でトークンを発行（`ntfy token add`、手作業）。トークンは全端末で 1 本を共有する。
    2. 設定元の設定画面にサーバホスト名・トークンを入力し、「鍵を作る」ボタンで共有鍵を生成する。
...
**フィルタは通知を出す側（発信側）の端末に効く設定である。** どの端末の画面から操作しても、
変わるのは発信側の設定で、その端末が何を・どう転送するかが決まる。受信端末からの操作は
コマンド（§3.4）で発信側へ届き、発信側のルールを書き換える。受信端末が自分の表示だけを変える
仕組みではないため、画面にもその旨を明示する（§10.4）。
...
ルール登録の主経路はタイムラインのコンテキストメニュー（§10.1）。受信端末からの登録は muteApp コマンド（§4）で転送元の denylist に反映される。一覧的な閲覧・編集はアプリフィルタ画面（§10.4）。

アプリごとの扱いは 1 つの値（`AppRuleSettings`）としてまとめて扱い、受信端末からの変更は
`setAppRule` コマンド（§3.4）でその値ごと送る。項目を増やしてもコマンドを増やさずに済み、
発信側の画面での編集と受信端末からの変更が同じ規則（`applyAppRule`）を通る。
...
- 受信端末で払いのけたときに元通知も消すか（§3.3）。判断は転送時にペイロード（`swipeBehavior`、§4.1）へ
  載せて配り、受信端末はそれに従う。設定を持つのが発信側なので、どの受信端末でも同じ挙動になる。既定は消さない
```
...
- 端末紛失時は、トークンを失効・再発行し、設定元で新トークンを入力 →「鍵を作る」→ 全端末で QR 読み直し（鍵ローテーションと同一手順）。
...
- **ディスクは単調増加しない**。メッセージは保持期間（既定 24 時間。高優先度と即時操作のコマンド（§3.4）は
  publish 時の `Cache` ヘッダで 60 秒へ短縮）、添付は 72 時間で自動的に消え、
  `attachment-total-size-limit` が天井になる。
...
- 通知アイテムには元通知のアクションボタン（`actions`）を表示する。`actionDetails`（§4.1）の分類が
  ...
  入力欄は開いた時点でフォーカスを受け、そのまま打ち始められる。トーストの返信ボタン（§3.3）から
  引き継いだときも同じ入力欄が開く。元通知が既に消えたアイテムは操作 UI を出さないため、引き継ぎを
  受けても開かない。
  ...
  （画像付き通知では「保存」が両方に現れうる）。**接頭辞は送信元の端末名で名指す**
  （`fromName`、無ければ `from`。発信元表示と同じ規則）。転送元はスマホとは限らず、どの端末も
  送信側になれる（§2）ため、固定の呼び名を使わない。発出元で画面が開く操作には、押しても手元には
  何も出ないことがわかる注記をさらに添える。分類できないアクションも接頭辞は付き、
  画面が開くかどうかの注記だけが落ちる。
  元通知が既に消えた（`sourceDismissed`）通知アイテムは、アクションボタン・返信入力・
  コンテキストメニューのアクション項目と「送信元の通知を消す」を出さない。
  「タイムラインから消す」「このアプリからの通知を非表示」は引き続き行える。
- **通知アイテムの吹き出し右上には、メニューボタンと元通知の状態を示すボタンが並ぶ。**
  **後者は状態を示すと同時に、その状態で意味のある操作を担う。**
  元通知が生きている間は ✓ を出し、押すと「元端末の通知を消す」command を送る（既読同期、§3.4）。
  元通知が消えた（`sourceDismissed`）あとは × を出し、押すとそのアイテムをこの端末のタイムラインから
  消す。消えたことを注記で伝えるのではなく記号で示すことで、状態表示と次の操作を 1 箇所にまとめる。
- **元通知が生きている通知は、タイムライン上部のバーからまとめて消せる。** 対象は表示中のうち
  元通知を指せて（`notificationKey` を持ち）まだ消えていないもので、同じ元通知の再投稿は 1 件として
  数える。対象が無いときはバーを出さない。全端末へ及んで取り消せないため、件数を示した確認を挟む。
  溜まった通知を朝にまとめて片づける操作を、1 件ずつ押さずに済ませるために置く。
- **「元端末の通知を消す」と「タイムラインから消す」は別の操作として分ける。** 前者は全端末へ及び、
  自端末のタイムラインにはアイテムを残す。後者は自端末のタイムラインからだけ消し、command を送らない。
  タイムラインから消したアイテムは表示から外れ、実体は剪定で落ちる（§11）。他端末のタイムラインには
  残る。トーストや通知からの操作（§3.3）はこの後者に到達しない。履歴を減らすのはタイムライン上でだけ
  行える操作とする。
- **アイテム種別に応じたコンテキストメニューは吹き出し右上のメニューボタンから開く。** Windows では
  右クリックでも開ける。本文の長押しは文字の選択に充てるため、メニューの入口には使わない。
  **メニューは開いた場所に出す。** ボタンからならその直下、右クリックならその座標。吹き出しを起点に
  すると、押した場所から遠いところに開いてしまう。
  通知アイテムなら「このアプリからの通知を非表示」（denylist 登録、受信端末からは muteApp コマンドで転送元に反映）・「送信元の通知を消す」・「タイムラインから消す」・アクション発火（インライン返信の分類は上記と同様に入力欄を開く）、（将来）メッセージアイテムなら「返信」等。
- **吹き出しの本文は文字を選んでコピーできる。** 選択範囲は 1 つの吹き出しの中で閉じる。一覧は画面外へ
  出た行を捨てるため、行をまたぐ選択は捨てられた時点で保てない。押して働く部品——アクションボタン・
  返信入力・コンテキストメニュー・添付の確認ダイアログ——は選択の対象から外す。
  **Windows は、選択があるときだけコンテキストメニューに「コピー」を並べる。** 選択の仕組みが持つ
  右クリックメニューをそのまま出すとタイムラインのメニューと重なるため、複写だけを引き取って
  1 つのメニューにまとめる。Android は長押しの選択が標準のツールバーを出すので、そちらに委ねる。
- **本文中のコードは押すだけでクリップボードへ入る。** 半角数字が 6 桁以上続く範囲をコードとみなし、
  URL と同じ見た目のリンクにする。押すとその文字列をクリップボードへ入れ、「コピーしました」を
  一時的に知らせる。続けて別のコードを押したときは前の知らせを引っ込め、最後に押したものの結果だけを
  見せる。5 桁以下は拾わない（金額の桁区切り・日付・時刻をリンクにしてしまう）。URL に含まれる数字も
  拾わない（URL は開く先であってコピーする値ではない）。受け取った認証コードだけを取り出す用途が主で、
  この規則から外れた文字列は上の文字選択で拾える。
...
設定元（「設定元にする」を選んだ端末）が QR を表示し、他の端末のカメラで
読み取る。フローの詳細は §6。
...
- 新端末: カメラで読み取るか、ペアリング文字列を手動で貼り付ける。**ペアリング文字列自体には端末名を
  ...
  する。未入力のまま取り込むこともでき、その場合は既存の端末名を引き継ぐ。**「端末名が未設定です」の
  警告は取り込み後の設定に端末名が無いときだけ出す**（入力欄の空欄だけを見ると、引き継いで名前が
  保たれている端末にも誤って警告することになる）。警告が出ても取り込みは完了し、後から設定画面で
  入力できる。
...
**この画面の設定はすべて発信側の端末に効く（§7）。** 受信端末から操作したときもコマンドで発信側へ
届いて発信側の設定が変わるため、画面の説明文でそのことを明示する。自端末の表示だけを変える設定と
取り違えると、操作の結果が理解できなくなる。
...
- タイムラインのコンテキストメニューから除外したアプリも、ここにチェック入りで現れる。
- 詳細（優先度上書き・redaction・払いのけの扱い）は**行末の 3 点ボタンから開く**。詳細はどちらの
  ロールでも同じダイアログで、扱いを 1 つの値としてまとめて保存する。行のどこかを押せば開くことは
  見て取れないため、開く場所を図柄で示す（アプリ名タップでも開くが、それだけを入口にしない）。
- 通知の自動転送を無効にした端末ではアプリ一覧が取れないため、履歴に現れたパッケージだけを候補にする。
  各行には**アイコンのトグルを並べて**扱いを切り替える（通知を送らせない / 払いのけたら発信側の通知も消す）。
  図柄だけでは意味が分からないため、一覧の上に同じ並び順の凡例を置く。トグルが 3 つ以上に増えるなら、
  行に並べるのをやめて 1 箇所へ畳む。
...
- 受信（Android）: ntfy アプリの導入と UnifiedPush ディストリビュータ選択 / ntfy 側の認証情報 / ntfy 側のバッテリー最適化除外の案内。
...
- UnifiedPush を使う端末には配送経路そのものを検査する2項目を置く。
  ...
  - **サーバ経由の受信テスト**（能動・手動実行）: 自分のエンドポイント topic へ平文マーカー `peranta-selftest:<nonce>` を publish し、5 秒以内の受信到達で合格とする。マーカーは受信口で横取りし、復号・タイムライン・OS 通知を一切経由しない。失敗は publish の HTTP ステータスで切り分け（403=ACL 不足、タイムアウト=ntfy アプリ側の認証未登録・省電力・購読断）、それぞれの対処を案内する。合格したときのアクセストークンの指紋を保存し、ntfy アプリの認証情報が今も通用するかの判定に使う（§10.6）。
...
[受信のセットアップを開く]で開く。受信経路（UnifiedPush）の手順 6 つを、状態に依らず同じ位置に常時
表示する作業台で、動作チェック（§10.5）の該当項目もこの画面へ誘導する（修復手段はここに一本化し、
動作チェック側は誘導のみ持つ）。

**手順の番号・タイトル・説明文は `ReceiveSetupSteps` が単一所有する。** 画面の文言から他の手順を
指すとき（「手順4の照合で」「手順2〜5を確認してください」等）も、番号を直に書かずこの定義から
組み立てる。直書きすると手順を 1 つ挿しただけで全ての参照が嘘になるため。

| # | 手順 | 常設内容 |
|---|---|---|
| 1 | ntfy アプリの導入 | 説明＋導入導線（済なら✓）。ntfy 以外のディストリビュータだけが居るときは、それを採用しない旨を事実として添える |
| 2 | ntfy にサーバを設定 | 手順文＋コピーチップ（デフォルトのサーバーに貼るサーバーURL）＋[ntfy を開く]。直接検査できないため状態は UnifiedPush 登録の照合結果から間接表示する |
| 3 | ntfy に認証情報を設定 | 手順文＋コピーチップ（カスタムヘッダーのサービスURL / ヘッダ名 / ヘッダ値）＋[ntfy を開く]。状態は受信テストの合格を根拠に出す（下記） |
| 4 | UnifiedPush の登録 | [登録する]/[登録し直す]＋エンドポイントのサーバ照合結果（不一致なら手順2を見直す案内） |
| 5 | ntfy を省電力から除外 | 説明＋設定導線 |
| 6 | 受信テスト | [テスト実行]/[再実行]＋結果 |

- バッジがどの状態でも、コピーチップやボタンなど道具は出続ける（未達でも作業台として機能する）。
- 画面復帰（ON_RESUME）ごとと、操作の直後（数回の自動再チェック）で状態を取り直す。

#### 認証情報の手順と、アクセストークンの再発行

**ntfy アプリの認証情報（カスタムヘッダー、またはユーザー管理）は Peranta の設定とは独立していて、
ペアリング QR の取り込みでは更新されない。** サーバのアクセストークンを再発行すると、Peranta 側だけが
新しいトークンになり、ntfy アプリは古いトークンのまま購読を続けようとして通らなくなる。配送だけが
静かに止まるため、独立した手順として立て、失効を状態で示す。

- 状態は**受信テストの合格を唯一の証拠として読み替える**。ntfy アプリの設定はこのアプリから覗けず、
  直接検査する手立てが無いため。
  - 今のアクセストークンで受信テストに合格している → ✓（配送が届いた＝資格情報が生きている）
  - 別のアクセストークンで合格していた → ✗（合格の根拠が失効したものとして再設定を促す）
  - 一度も合格していない → 未確認
- そのために、**受信テストに合格した時点のアクセストークンの指紋**（SHA-256、§11）を 1 つだけ保存する。
  トークンそのものは複製せず、ログにも出さない（§6）。
- 受信テストが**タイムアウト**したとき、前回の合格時からトークンが変わっていれば、汎用の「手順2〜5を
  確認してください」ではなく、ntfy アプリの認証情報を新しいトークンで登録し直す旨とこの手順への
  誘導を出す。全ての手順が ✓ のまま「どこかを確認してください」と言われる状態を作らない。
- publish 自体が失敗した結末（HTTP 拒否・接続失敗）は配送以前の問題なので、トークンが変わっていても
  汎用の案内のままにする。
...
4. **受信手順**（UnifiedPush 経由で受信する端末のみ。現状 Android）: §10.6 の手順（ntfy アプリの
   導入 → ntfy にサーバを設定 → ntfy に認証情報を設定 → UnifiedPush の登録 → ntfy を省電力から
   除外 → 受信テスト）を自動転送の可否で出し方を変える。自動転送する端末は全手順を
   「逆方向チャネルの受信経路」として 1 ページに集約し、
   あとで設定することができる（スキップ時は「他の端末からこの端末の通知を消したり既読にしたり
...
（あとで設定できるページだけ「あとで設定する」で飛ばせる）。ただし **ntfy アプリ側の設定を直接
検査できない手順（ntfy にサーバを設定・ntfy に認証情報を設定）は、未確認のままでも次へ進める**。
どちらも後続の手順（エンドポイントの照合・受信テスト）でしか裏を取れず、未確認で止めると
そこへ辿り着けなくなるため。要対処（✗）と判定されたときは他と同じく止める。
入力・操作は全て即時保存する。
...
| 受信テスト合格時のトークン指紋 | string | なし | 受信テストに最後に合格したときのアクセストークンの SHA-256（16 進）。ntfy アプリの認証情報が今のトークンで通用するかの判定に使う（§10.6）。**トークンそのものは複製せず**、指紋からは復元できないため秘密としては扱わないが、ログには出さない |

### ウィンドウの見え方（Desktop）

前回終了時のウィンドウの位置・大きさ・最大化状態を覚え、次回の起動時に復元する。設定と同じストア
（`to/sava/peranta` ノード）へ保存するが、この端末でだけ意味を持つ表示状態のためペアリングでは配らない。
...
- 置き場所はアプリ専用領域とし、権限やストレージ制限の影響を受けない場所を使う:
  ...
  - パス解決は expect/actual。
- 剪定: 上限件数（既定 1000 件）の超過分と期限切れ（`expiresAtEpochMillis`）のエントリ、および
  タイムラインから消したエントリ（§10.1）は、起動時にファイル書き直しで削除する。
...
ジョブ間で受け渡す成果物には `retention-days` を明示する。Actions のストレージ枠はリポジトリ単位では
なくアカウント全体で共有するため、既定の 90 日で抱え続けると他のリポジトリの CI まで止まる。課金は
容量と保持時間の積で決まるので、期間を切るのが最も効く。

- **MSI とマニフェストは 1 日。** どちらもタグを打ったときだけ作られ、`release` ジョブが公開すれば
  リリースに実物が残る。成果物として要るのは受け渡しの間だけ。
- **APK はタグを打った回が 1 日、手動実行の回が 3 日。** タグを打った回はリリースへ載るので受け渡しの
  間だけでよい。手動実行の回は成果物としてしか残らないため、実機へ入れるまでの猶予を置く。

リリースの asset はこのストレージ枠の外にあるため保持期間の影響を受けず、アプリが `latest` から
配布物を引く経路は変わらない。
...
- Android: 新版があれば APK をダウンロードし、sha256 と packageName を確かめてから、`ACTION_VIEW` の Intent でパッケージインストーラへ引き渡して更新を促す（`REQUEST_INSTALL_PACKAGES` 権限、FileProvider 経由）。インストールの可否はユーザーが確認画面で決める。Play ストア外の配布物なので Play プロテクトの警告を経由する。
