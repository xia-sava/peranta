...
ルール登録の主経路はタイムラインの長押しメニュー（§10.1）。受信端末からの登録は muteApp コマンド（§4）で転送元の denylist に反映される。一覧的な閲覧・編集はアプリフィルタ画面（§10.4）。
...
- **通知アイテムの吹き出し右上のボタンは、元通知の状態を示すと同時に、その状態で意味のある操作を担う。**
...
- **「元端末の通知を消す」と「タイムラインから消す」は別の操作として分ける。** 前者は全端末へ及び、
  ...
  行える操作とする。
- 長押し（Windows は右クリック）でアイテム種別に応じたコンテキストメニューを開く。通知アイテムなら「このアプリからの通知を非表示」（denylist 登録、受信端末からは muteApp コマンドで転送元に反映）・「送信元の通知を消す」・「タイムラインから消す」・アクション発火（インライン返信の分類は上記と同様に入力欄を開く）、（将来）メッセージアイテムなら「返信」等。
