...
/**
 * Desktop の復号済み添付キャッシュ（§4.3）。blob ごとに `<baseDir>/<blobId>/<サニタイズ済みfileName>` へ置く。
 * ダウンロードは必ず一時ファイルへ復号し、全チャンクの検証が完走してから最終ファイル名へ rename する
 * （持ち越し事項 1）。復号に失敗したら一時ファイルを消して例外を送出し、部分ファイルを残さない。
 * [sharedKey]/[keyId] は [AttachmentRef.enc] の検証と blobKey 導出に使う。
 */
class DesktopAttachmentCache(
    private val transport: BlobTransport,
    private val sharedKey: ByteArray,
    private val keyId: String,
    private val baseDir: File = JvmPaths.attachmentsDir,
    private val now: () -> Long = { System.currentTimeMillis() },
    private val log: Logger = Logger.withTag("AttachmentCache"),
) {
    ...
    private fun blobDir(ref: AttachmentRef): File = File(baseDir, sanitizeAttachmentFileName(ref.blobId, fallback = "blob"))
